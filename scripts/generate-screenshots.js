...
async function generateScreenshots() {
  ...
  try {
    ...
    for (const viewport of viewports) {
      console.log(
        `📱 Génération capture ${viewport.name} (${viewport.config.width}x${viewport.config.height})...`
      );
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, waitConfig.animationDelay)
      );

      // Attendre que les images se chargent
      await new Promise((resolve) =>
        setTimeout(resolve, waitConfig.imageLoadDelay)
      );
      ...
      if (fs.existsSync(screenshotPath)) {
        ...
        console.log(
          `✅ Capture ${viewport.name} sauvegardée: ${viewport.filename} (${sizeKB}KB)`
        );
      } else {
