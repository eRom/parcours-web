...
async function runLighthouseTest(url, config, configName) {
  ...
  try {
    ...
    const reportPath = path.join(
      reportDir,
      `lighthouse-${configName.toLowerCase()}-${timestamp}.json`
    );
    ...
    if (audits["largest-contentful-paint"]) {
      ...
      console.log(
        `📈 LCP: ${lcp.displayValue} (${lcp.score >= 0.9 ? "🟢" : lcp.score >= 0.5 ? "🟡" : "🔴"})`
      );
    }
    ...
    if (audits["first-input-delay"]) {
      ...
      console.log(
        `⚡ FID: ${fid.displayValue} (${fid.score >= 0.9 ? "🟢" : fid.score >= 0.5 ? "🟡" : "🔴"})`
      );
    }
    ...
    if (audits["cumulative-layout-shift"]) {
      ...
      console.log(
        `📐 CLS: ${cls.displayValue} (${cls.score >= 0.9 ? "🟢" : cls.score >= 0.5 ? "🟡" : "🔴"})`
      );
    }
    ...
    opportunities.forEach(([key, audit]) => {
      const savings =
        audit.displayValue || `${Math.round(audit.numericValue)}ms`;
    ...
    return {
      ...
      scores: Object.fromEntries(
        Object.entries(lhr.categories).map(([key, data]) => [
          key,
...
async function main() {
  ...
  if (device === "desktop" || device === "both") {
    const desktopResult = await runLighthouseTest(
      url,
      configs.desktop.settings,
      configs.desktop.name
    );
    results.push(desktopResult);
  }

  if (device === "mobile" || device === "both") {
    const mobileResult = await runLighthouseTest(
      url,
